...
namespace Unity.Physics.Stateful
{
    ...
    public static class StatefulEventCollectionJobs
    {
            ...
            where C : unmanaged, IComponentData
        {
            ...
            public ComponentLookup<HasPhysicsEvents> EventHasEvenLookup;
            public void Execute()
            {
                ...
                for (int i = 0; i < statefulEvents.Length; i++)
                {
                    ...
                    if (addToEntityA)
                    {
                        EventHasEvenLookup.SetComponentEnabled(statefulEvent.EntityA, true);
                        EventBuffers[statefulEvent.EntityA].Add(statefulEvent);
                    }
                    if (addToEntityB)
                    {
                        EventHasEvenLookup.SetComponentEnabled(statefulEvent.EntityB, true);
